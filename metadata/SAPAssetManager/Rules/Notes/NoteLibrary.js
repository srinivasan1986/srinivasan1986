...
export class NoteLibrary {
    ...
    static didSetNoteTypeTransactionForBindingType(clientAPI) {
        ...
        switch (bindingType.substr(startIndex)) {
            case 'MyWorkOrderTool': 
                this.setNoteTypeTransactionFlag(clientAPI, TransactionNoteType.prt(binding['@odata.readLink']));
                break;
    ...
    static  didSetNoteTypeTransactionFlagForPage(clientAPI, page) {
        switch (page) {
            case 'PRTListViewPage':
                this.setNoteTypeTransactionFlag(clientAPI, TransactionNoteType.prt(clientAPI.binding.Tools[0]['@odata.readLink']));
                break;
    ...
    static createLinks(context) {
        ...
        if (onChangeSet) {
            ...
            if (objectType.readLink) {
                readLink = objectType.readLink;
            }
