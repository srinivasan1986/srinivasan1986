...
export default function BusinessPartnerIsValid(context) {
    ...
    // Clear previous 
    clearInLineErrors(context, controls);
 ...
 */
export function clearInLineErrors(context, controls) {
    for (let item of controls) {
        libCom.setInlineControlErrorVisibility(item, false);
    }
...
export function invalidateControls(context, controlMap, errorMap) {
    ...
    Object.keys(errorMap).forEach((item) => {
        libCom.setInlineControlError(context, controlMap[item], errorMap[item]);
    });
    context.getControl('FormCellContainer').redraw();
    return false;
}
