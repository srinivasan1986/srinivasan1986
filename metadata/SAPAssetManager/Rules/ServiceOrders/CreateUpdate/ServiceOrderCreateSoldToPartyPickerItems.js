...
export default function ServiceOrderCreateSoldToPartyPickerItems(context) {
    ...
    if (binding.WOSales_Nav) {
        ...
        if (salesOrg && distributionChannel && division) {
            ...
            target.setReturnValue('{Customer}');
            target.setQueryOptions(`$filter=SalesOrg eq '${salesOrg}' and DistributionChannel eq '${distributionChannel}' and Division eq '${division}'&$expand=Customer_Nav&$orderby=Customer_Nav/Name1`);
        }
    }
