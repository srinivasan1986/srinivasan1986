...
export default class {
    ...
    static reloadUserTimeEntries(context, returnEmpty=false, clockOut, businessObject, execute=true) {
     ...
     /**
     * Is the user currently clocked in to a work order or operation?  New code to handle multi-device
     * @param {*} context
     */
      static isUserClockedIn(context) {

        if (libThis.isCICOEnabled(context)) {
            let opAssign = libMobile.isOperationStatusChangeable(context);
            let woAssign = libMobile.isHeaderStatusChangeable(context);
            let entitySet;
            let statusNav;

            if (!opAssign && !woAssign) {
                return Promise.resolve(false); //assignment model is not header or operation
            }

            /**
             * //TODO - Use a state variable in the future to speed things up
            //let pageName = libCom.getPageName(context);
            //if (pageName === 'WorkOrdersListViewPage' || pageName === 'WorkOrderOperationsListViewPage') {
                let isClockedIn = libCom.getStateVariable(context, 'IsUserClockedIn');
                if (isClockedIn) {
                    return Promise.resolve(isClockedIn === 'Y');
                }
            //}
            */
            ...
            if (opAssign) { //Operations
                entitySet = 'MyWorkOrderOperations';
                statusNav = 'OperationMobileStatus_Nav/MobileStatus';
            } else { //Work orders
                entitySet = 'MyWorkOrderHeaders';
                statusNav = 'OrderMobileStatus_Nav/MobileStatus';
            }
            let filter = '$select=UserTimeEntry_Nav/PreferenceGroup,OrderId&$expand=UserTimeEntry_Nav&$filter=sap.entityexists(UserTimeEntry_Nav) and ' + statusNav + " eq '" + startStatus + "'&$orderby=UserTimeEntry_Nav/PreferenceValue desc";
            return context.read('/SAPAssetManager/Services/AssetManager.service', entitySet, [], filter).then(function(results) {
                if (results && results.length > 0) {
                    let clockedIn = false;
                    results.every(function(row) { //Loop over all the STARTED rows
                        if (row.UserTimeEntry_Nav[0].PreferenceGroup === 'CLOCK_IN') {
                            clockedIn = true; //CLOCK_IN is the newest record in the collection, so user is clocked in to this object.
                            return false; //Breaks out of the loop
                        }
                        return true; //Continue loop
                    });
                    //libCom.setStateVariable(context, 'IsUserClockedIn', clockedIn === true ? 'Y': 'N');
                    return clockedIn;
                }
                //libCom.setStateVariable(context, 'IsUserClockedIn', 'N');
                return false; //No STARTED business objects
            }).catch(() => {
                Logger.error(context.getGlobalDefinition('/SAPAssetManager/Globals/Logs/CategoryClockInOut.global').getValue(),'ClockInClockOutLibrary.isUserClockedIn: UserTimeEntries read failure.');
                return Promise.resolve(false); //Read failure
            });
        }
        return Promise.resolve(false); //CICO feature not enabled
    }
