...
*/
    ...
    static checkPasscodeLength(context, control) {
        ...
        if (libVal.evalIsEmpty(passcodeText)) {
            // show inline error 
            libCom.setInlineControlError(context, control, context.localizeText('passcode_is_mandatory'));
            return Promise.reject(false);
        } else {
            if (passcodeText.length < 6) {
                libCom.setInlineControlError(context, control, context.localizeText('passcode_length'));
                return Promise.reject(false);
            }
        }
        return Promise.resolve(true);
